...
API のソート順は会議開催日の降順固定で、`maximumRecords` 部分取得は新しい側 N 件のみが返る。加えて **同日内（同一会議内）は `speechOrder` 昇順** で返る（実測ベース、公式仕様未掲載）ため、最古日付確定後に `maximumRecords=1` で 1 件取得しても、それは「同日内の最終発言（`speechOrder` 最大）」であり真の初発言ではない。
...
API は会議開催日の降順固定で、同日内は `speechOrder` 昇順（実測）。最古日付確定後に `maximumRecords=1` のみで取得する従来手順 (API 単独) では:

- 期間内最古の日付の **最終発言**（同日 `speechOrder` 最大）を取得 ← 真の初発言ではない

実証: 松岡克由の 1972-06-08 当日は 10 件 (`speechOrder=32〜51`)。`maximumRecords=1` (API 降順固定) だと `speechOrder=51` の最終発言を取得してしまう。`--sort date-asc,speech-order-asc` でクライアント側ソートをかけることで、`speechOrder=32` の真の初発言が先頭に来る。
