...
`speakerRole` の不正値（例: `証人` / `参考人` / `公述人` 以外の文字列）は HTTP 400 で返るが、具体的なエラーコードは公式に明示されておらず `19011`（入力誤り）相当と推定される。同じ列挙型でも `nameOfHouse` の不正値は HTTP 200 で silently 無視される点と対照的（前述の非対称性を参照）。
