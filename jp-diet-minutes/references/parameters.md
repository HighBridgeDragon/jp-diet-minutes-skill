...
公式仕様より、API が返すエラー番号の主要例。これらの番号は独立した JSON フィールドではなく、`message` / `details` 文字列内に `(19011)` のように埋め込まれて返る（実 JSON 構造は下記サンプルを参照）。詳細なエラーレスポンス形式は [api-reference.md](./api-reference.md#エラーレスポンス) を参照。
