...
`sessionFrom`/`sessionTo` および `issueFrom`/`issueTo` の「単独指定で完全一致」は公式仕様（<https://kokkai.ndl.go.jp/api.html>）に明記された挙動。実測検証は M3 #12 で行う予定。
...
`maximumRecords` の上限・既定値はエンドポイントごとに異なる（前掲）。それ以外の全パラメータは 3 エンドポイント共通。
...
| `19011` | 入力誤り（型不一致など総合的なエラー。詳細フィールドコード `1902x` 系を `details` 配列で内包） |
| `19012`〜`19020` | 日付形式・値の妥当性、入力文字数超過などの個別エラー |
| `19028` | `speakerRole` の列挙値不正（`証人` / `参考人` / `公述人` 以外）。実測でトップレベル `19011` の `details` に内包 |

`speakerRole` の不正値（例: `証人` / `参考人` / `公述人` 以外の文字列）は HTTP 400 で返り、実測では以下のレスポンス body が返る:

```json
{
  "message": "(19011)検索条件の入力に誤りがあります。",
  "details": [
    "(0) (19028)speakerRole:発言者役割を証人/参考人/公述人で入力してください。"
  ]
}
```

同じ列挙型でも `nameOfHouse` の不正値は HTTP 200 で silently 無視される点と対照的（前述の非対称性を参照）。
