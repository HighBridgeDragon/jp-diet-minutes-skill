...
- 院名・会議名・キーワード・発言者名・期間・回次・号数・各種 ID のいずれかを **最低 1 つ指定する必要がある**（指定なしはエラー番号 `19007` が返る）
...
| パラメータ | 型 | 既定値 | 検索方式 | 複数指定 | 備考 |
|---|---|---|---|---|---|
| `nameOfHouse` | `衆議院` / `参議院` / `両院` / `両院協議会` | （指定なし） | 完全一致（列挙） | 不可 | `両院` と `両院協議会` は同義（エイリアス）。どちらを指定しても両院協議会の会議録が返る（実測ベース）。**不正値は無言で無視され、フィルタなしの結果が返る**（HTTP 200） |
...
「[共通ルール](#共通ルール)」の通り複数パラメータは AND 評価。ただし `any`（発言本文）と `speaker`（発言者属性）のように **検索対象軸が異なる条件** を併用すると、結果は両者の積集合（intersection）となり、一方単独より大幅に件数が減る。意図せず両方を併用すると、件数の絞り込みが誤診を招く。

実証例（2026-06-16 観測、`from` / `until` 等の追加条件なし）:

| クエリ | `numberOfRecords` |
|---|---:|
| `speaker=石原慎太郎` | 2,098 |
| `any=石原慎太郎` | 2,143 |
| `speaker=石原慎太郎&any=石原慎太郎` | 647 |

公式仕様（<https://kokkai.ndl.go.jp/api.html>）には `any` 単独・`speaker` 単独の挙動のみ明示されており、両者の併用挙動は記載がない。本挙動は実測ベース。

意図別の使い分け（詳細は [recipes/speaker-vs-any-disambiguation.md](./recipes/speaker-vs-any-disambiguation.md) 参照）:

- 議員 A の **全発言** を取得 → `speaker=A` 単独（`any=A` を併用しない）
- 議員 A への **言及**（第三者の発言を含む）を取得 → `any=A` 単独（`speaker=A` を併用しない）
- 議員 A 本人の **自己言及** に限定 → `speaker=A&any=A` の積集合を意図的に活用
...
公式仕様より、API が返すエラー番号の主要例。これらの番号は独立した JSON フィールドではなく、`message` / `details` 文字列内に `(19011)` のように埋め込まれて返る（実 JSON 構造は下記サンプルを参照）。**`details` フィールドはエラー種別により省略される**（例: `19007` では `details` キー自体が存在せず `message` のみ）。詳細なエラーレスポンス形式は [api-reference.md](./api-reference.md#エラーレスポンス) を参照。
...
| `19011` | 入力誤り（型不一致など総合的なエラー。詳細フィールドコード（実測例: `19028`）を `details` 配列で内包） |
| `19012`〜`19020` | 日付形式・値の妥当性、入力文字数超過などの個別エラー |
| `19028` | `speakerRole` の列挙値不正（`証人` / `参考人` / `公述人` 以外）。実測でトップレベル `19011` の `details` に内包。**[公式仕様](https://kokkai.ndl.go.jp/api.html)の「表 2：エラーメッセージ」には未掲載**で、実装上のみ確認されている拡張コード |

公式仕様の表 2 に掲載されているコードは `19001` / `19004` / `19005` / `19006` / `19007` / `19011` / `19012` / `19013` / `19014` / `19018` / `19019` / `19020` のみ。実 API は `19028` のような詳細コードを `details` 配列で追加で返してくる場合があるが、これは仕様に明文化されていないため、エラーハンドラ実装時は `19028` 等の存在を前提とせず、トップレベル `19011` でフォールバックできる構成にすること。

#### エラーハンドラ実装指針

- `details` フィールド（JSON 配列・XML `<details>` 要素）は **エラー種別により省略される**。`19007`（必須欠落）のように `message` のみが返るケースを必ず想定する
- `body.details[0]` のような無条件アクセスは `19007` 等で `undefined` / `KeyError` を踏むため避ける
- `details` の有無を条件分岐し、トップレベル `message` 内の `(19011)` 形式のコードのみで分類できる構造にすること（`details` 内の詳細コードはあくまで補助情報として扱う）
