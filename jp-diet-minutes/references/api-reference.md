...
| 列挙値の不正 — `speakerRole`（`証人` / `参考人` / `公述人` 以外） | 400 | エラー本文を返す（トップレベル `errorCode` `19011`、`details` に `19028` を内包。`19028` は[公式仕様](https://kokkai.ndl.go.jp/api.html)の「表 2：エラーメッセージ」未掲載・実測のみ） |
