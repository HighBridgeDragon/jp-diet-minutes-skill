...
| `nameOfHouse` | 文字列 | 常に | `衆議院` / `参議院` / `両院`（両院協議会の会議録もこの値で返る）|
...
> [!NOTE]
> `numberOfReturn: 2` のうち 2 件目の `meetingRecord` は紙面節約のため省略している。
...
- 改行を `\n` に正規化したい場合は `\r\n` → `\n` 置換
- 発言冒頭の `○` + 肩書き + 全角スペースのパターンは正規表現 `^○[^　]+　` で除去可能（末尾は全角空白 U+3000）
...
`目次` / `索引` / `附録` / `追録` も同じ検索クエリでヒットしうる。発言本文の調査時は **クライアント側で `imageKind === "会議録"` フィルタを必須**とする。

なお `supplementAndAppendix` / `contentsAndIndex` パラメータは **除外用ではなく限定用**（実 API 検証済み）:

- `supplementAndAppendix=true`: **追録・附録のみに限定**（その他を除外）
- `contentsAndIndex=true`: **目次・索引のみに限定**（その他を除外）
- いずれも `=false` は既定値であり、明示しても何のフィルタも掛からない（除外目的では機能しない）。詳細は [parameters.md](./parameters.md) を参照。
