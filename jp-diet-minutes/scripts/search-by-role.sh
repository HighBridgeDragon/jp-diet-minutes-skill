...
show_help() {
  cat <<'HELP'
search-by-role.sh — 役割で発言検索 (GET /api/speech, speakerRole)

Usage:
  bash scripts/search-by-role.sh <role> [from] [until] [limit] --sort <keys>

Positional arguments:
  role            証人 / 参考人 / 公述人 のいずれか（それ以外は API 側で HTTP 400）
...
if [ -z "$ROLE" ]; then
  echo "Usage: bash scripts/search-by-role.sh <role> [from] [until] [limit] --sort <keys>" >&2
