...
claude.ai で実際に利用するには、組織のオーナーまたはユーザーのネットワーク設定で `kokkai.ndl.go.jp` を許可ドメインに追加する必要があります。`jq` 非依存化は別途対応が必要です。いずれも満たせない環境では、Claude Code 経由（`npx skills add`）をご利用ください。
